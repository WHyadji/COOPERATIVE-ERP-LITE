...
test.describe('Member Portal - Login Flow', () => {
  test('should display login page correctly', async ({ page }) => {
    ...
    await expect(page.getByText('Masuk ke portal anggota koperasi')).toBeVisible();
  ...
  test('should login successfully with valid credentials', async ({ page }) => {
    ...
    await expect(page.getByText(`Selamat Datang, ${TEST_MEMBER.namaLengkap}`)).toBeVisible();
  });
...
test.describe('Member Portal - Transactions Page', () => {
  ...
  test('should filter transactions by type', async ({ page }) => {
    ...
    const chips = await page.$$('[class*="MuiChip"]:has-text("Simpanan Pokok")');
...
test.describe('Member Portal - Profile Page', () => {
  ...
  test('should display profile header', async ({ page }) => {
    ...
    await expect(page.getByText(`Nomor Anggota: ${TEST_MEMBER.nomorAnggota}`)).toBeVisible();
  });
...
test.describe('Member Portal - Navigation', () => {
  ...
  test('should logout successfully', async ({ page }) => {
    // Click user menu
    await page.click('[aria-label="account of current user"]', { timeout: 5000 }).catch(() => {
  ...
  test('should show mobile menu on small screens', async ({ page }) => {
    ...
    const menuButton = page.locator('[aria-label="open drawer"]').or(page.locator('button:has([data-testid="MenuIcon"])'));
