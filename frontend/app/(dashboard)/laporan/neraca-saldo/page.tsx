  ...
  useEffect(() => {
    ...
    const fetchReport = async () => {
      try {
        ...
        if (!ignore)
          setError(
            err instanceof Error ? err.message : "Gagal memuat neraca saldo"
          );
      } finally {
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          <Box>
            ...
            <Typography color="text.secondary">Trial Balance</Typography>
          </Box>
        </Box>
        ...
        <>
          <Grid container spacing={2} sx={{ mb: 3 }} className="no-print">
            ...
            <Grid item xs={12} md={4}>
              <Card
                sx={{
                  bgcolor: report.isBalanced ? "success.main" : "error.main",
                  color: "white",
                }}
              >
                <CardContent>
          ...
          <Paper>
            <TableContainer>
              <Table>
                <TableHead>
                  <TableRow>
                    <TableCell width="100">Kode Akun</TableCell>
                    <TableCell>Nama Akun</TableCell>
                    <TableCell width="120">Tipe</TableCell>
                  ...
                  {report.items.map((item, i) => (
                    <TableRow key={`item-${i}`} hover>
                      ...
                      <TableCell>
                        <Chip
                          ...
                          color={
                            item.tipeAkun === "aset"
                              ? "primary"
                              : item.tipeAkun === "kewajiban"
                                ? "warning"
                                : item.tipeAkun === "modal"
                                  ? "secondary"
                                  : item.tipeAkun === "pendapatan"
                                    ? "success"
                                    : "error"
                          }
                        />
                      ...
                      <TableCell align="right">
                        {item.saldoDebit > 0
                          ? formatCurrency(item.saldoDebit)
                          : "-"}
                      </TableCell>
                      <TableCell align="right">
                        {item.saldoKredit > 0
                          ? formatCurrency(item.saldoKredit)
                          : "-"}
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            <Box sx={{ p: 2 }}>
              <Chip
                label={report.isBalanced ? "Balanced ✓" : "Unbalanced ✗"}
